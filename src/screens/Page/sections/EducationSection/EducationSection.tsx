      ...
      <div className={`${itemGap}`} ref={ref}>
        ...
        >
          {education.items.map((item, index) => (
            <motion.div
              key={index}
              className={`group relative border-l-2 border-gray-300 pl-16 md:pl-20 lg:pl-24 hover:border-purple-500 transition-all duration-500 dark:border-gray-600 dark:hover:border-purple-400 ${normalTransition} rounded-r-lg`}
              variants={staggerItem}
              whileHover={{ x: 4 }}
            >
              {/* Timeline dot - animated on scroll */}
              <motion.div
                className="absolute left-0 top-0 w-5 h-5 bg-gray-400 rounded-full border-4 border-white group-hover:bg-purple-500 transition-all duration-500 transform -translate-x-2.5 dark:bg-gray-600 dark:border-black dark:group-hover:bg-purple-400"
                initial={{ scale: 0 }}
